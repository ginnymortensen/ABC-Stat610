...
sampled_data <- sample_from_matrix_distribution(W, obs_matrix)
print(sampled_data)
```
...
```{r}
abc_sim(obs_matrix4, prior, 30, 10000)

```


```{r}
abc_sim_both(obs_matrix1, obs_matrix2, prior, 30, 10000)
```
...
```{r}
abc_smc(obs_matrix2, prior, c(80, 50, 30, 20, 15, 13, 12), 1000)
```
```{r}
...
```{r}
abc_smc_both(obs_matrix1, obs_matrix2, prior, c(80, 50, 30, 20, 15, 13, 12), 1000)
```

```{r}
abc_smc_both(obs_matrix3, obs_matrix4, prior, c(40, 20, 15, 10, 8, 6, 5), 10000)
```
